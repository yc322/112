...
int main (int argc, char ** argv)
{
  ...
  file.my_mkfs ();
  if (file.my_mkdir ("/foo") < 0) {  /* already tested before */
    printf ("second or subsequent test\n");
    first_test = 0;
  }
  if (file.my_rmdir ("/foo") < 0) {
    if (first_test) {
  ...
  if (file.my_open ("/foo/bar") >= 0) {	/* file should not exist */
...
static int test_file (char * path, char * buffer, int size, int max_size)
{
  ...
  /* now read and write */
  if ((fd = file.my_open (path)) < 0) { /* file should exist now */
    printf ("error, unable to reopen %s, aborting\n", path);
    exit (1);
  }
  printf ("successfully reopened file %s\n", path);
  if (file.my_write (fd, buffer, size / 2) != size / 2) {
  ...
  if (file.my_read (fd, buffer + size / 2, size - size / 2) != size - size / 2) {
