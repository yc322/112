...
#include <iostream>
enum Error_code{success,overfloat,not_present};
...
enum Error_code {not_present, duplicate_error, success};
...
public:
    ...
    Error_code tree_search(Record &target) const;
    Error_code search_and_detroy(Binary_node<Record> *&sub_root,const Record &target);
...
template <class Record>
Error_code Search_tree<Record>::search_and_detroy(Binary_node<Record> *&sub_root,const Record &target)
{
    if (sub_root==NULL || sub_root->data==target)
